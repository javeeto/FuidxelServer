...
//Cambia de formato fecha del tipo d/m/Y a d-m-Y de informix
function formato_fecha_informix($fecha_esp) {
    if (isset($fecha_esp) &&
            trim($fecha_esp) != '') {
        $fechaarray = explode("/", $fecha_esp);
        $yyyy = $fechaarray[2];
        $mm = $fechaarray[1];
        $dd = $fechaarray[0];
        return $dd . "-" . $mm . "-" . $yyyy;
...
function validar_diferencia_fechas($fechainicial, $fechafinal, $mensajetrue = 0) {
    ...
    $dd_ini = $fechainicial[0] . $fechainicial[1];
    $dd_fin = $fechafinal[0] . $fechafinal[1];
    $mm_ini = $fechainicial[3] . $fechainicial[4];
    $mm_fin = $fechafinal[3] . $fechafinal[4];
...
//Resta dias a una fecha con formato mysql
...
function fechaVacia($fecha) {
    if (trim($fecha) == '' ||
            substr($fecha, 1, 10) == '01/01/1970') {
        $retornafecha = '01/01/1970';
    } else {
        $retornafecha = $fecha;
    }
    return $retornafecha;
...
//Valida de formato fecha del tipo d/m/Y 
function validarFecha($fecha) {
    $patron = ";(0[1-9]|[12][0-9]|3[01])[".preg_quote("\/")."](0[1-9]|1[012])[".preg_quote("\/")."](19|20)[0-9]{2};";
